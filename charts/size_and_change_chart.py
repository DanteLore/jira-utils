...
class SizeAndChangeChart(JiraChart):
    ...
    def save_to_file(self, filename):
        ...
        ax.bar(ind, self.negative_data, width=width, linewidth=0, facecolor="#CC0000")
        plt.xticks(ind + width / 2, self.x_data)

        ax.bar(ind, self.positive_data, width=width, linewidth=0, facecolor=self.ctm_green)

        plt.xticks(ind + width / 2, self.x_data)
        ax2.plot(ax.get_xticks(), self.size_data, linestyle='-', linewidth=4.0, color=self.ctm_light_blue)
